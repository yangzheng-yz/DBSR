...
def main():
    ...
    for idx_traj, permutation in enumerate(permutations):
        ...
        data_processing_val = processing.SyntheticBurstDatabaseProcessing(cfg.crop_sz, cfg.burst_sz,
    ...
    with open('results_of_%s-%s.pkl' % (args.ckpt_path.split('/')[-1].split('.')[0], args.trajectory_path.split('/')[-1].split('.')[0]), 'wb') as f:
