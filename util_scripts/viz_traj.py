...
grid_size = 3 # 3 for 1/4, 6 for 1/8, 9 for 1/12
burst_sz = 4
traj_root_path = "/mnt/7T/zheng/DBSR_results/loggings/b4_1-4_20231022_v6"
f = open(os.path.join(traj_root_path, 'traj_int_length_0.25.pkl'), 'rb')
...
# 计算加权频次
trajectory_strs = [''.join(map(str, traj.flatten())) for traj in pixel_shift_trajectories]
counts = Counter()
...
# 获取前5个最常见的轨迹
common_trajectories = counts.most_common(5)
...
plt.savefig(os.path.join(traj_root_path, 'top_5_trajectories.png'))
