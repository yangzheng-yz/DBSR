...
def run(settings):
    settings.description = 'Default settings for training DBSR models on synthetic burst dataset(NightCity) with step(4), amplify factor(4), crop size(384,384), specific translation(No.322)'
    ...
    f = open("/home/yutong/zheng/projects/DBSR/util_scripts/trajectory_step-4_range-4.pkl", 'rb')
    permutations = pkl.load(f)
    f.close()
    
    permutation = permutations[322]
