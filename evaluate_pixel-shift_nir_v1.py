"""
This version is for the nir synthetic dataset.
"""
...
def main():
    ...
    nir_synthetic_test = datasets.nir_synthetic(root=cfg.dataset_path, split=cfg.split)   
    ...
    scores_all_mean = {}
    selected_images_id = np.arange(0, 340) 
    ...
    for idx_traj, permutation in enumerate(permutations):
        ...
        if os.path.exists(os.path.join(dir_path, 'nir_synthetic_%s_meta_infos.pkl' % cfg.split)):
            with open(os.path.join(dir_path, 'nir_synthetic_%s_meta_infos.pkl' % cfg.split), 'rb') as f:
                meta_infos_val = pkl.load(f)
            print(" *Using the predefined ISP parameters in %s" % os.path.join(dir_path, 'nir_synthetic_%s_meta_infos.pkl' % cfg.split))
        ...
        data_processing_val = processing.SyntheticBurstDatabaseProcessing((cfg.crop_sz[0], cfg.crop_sz[1]), cfg.burst_sz,
        ...
        dataset_val = sampler.IndexedImage(nir_synthetic_test, processing=data_processing_val)
        ...
        for idx, data in enumerate(dataset_val):
            ...
            if meta_infos_found:
                ...
                with open(os.path.join(dir_path, 'nir_synthetic_%s_meta_infos.pkl' % cfg.split), 'wb') as f:
