...
class BaseJointTrainer:
    """Base trainer class. Contains functions for training and saving/loading chackpoints.
    Trainer classes should inherit from this one and overload the train_epoch function."""

    def __init__(self, actors, loaders, sr_optimizer, agent_optimizer, settings, sr_lr_scheduler=None, agent_lr_scheduler=None):
        """
        args:
        ...
        self.sr_optimizer = sr_optimizer
        self.sr_lr_scheduler = sr_lr_scheduler
        self.agent_lr_scheduler = agent_lr_scheduler
        ...
        for i in range(num_tries):
            try:
                ...
                for epoch in range(self.epoch+1, max_epochs+1):
                    ...
                    if self.sr_lr_scheduler is not None:
                        self.sr_lr_scheduler.step()
                    if self.agent_lr_scheduler is not None:
    ...
    def save_checkpoint(self):
        """Saves a checkpoint of the superresolution network as well as agent network and other variables."""
    ...
    def load_checkpoint(self, checkpoint = None, fields = None, ignore_fields = None, load_constructor = False):
        ...
        if checkpoint is None:
            ...
            checkpoints_path = []
            assert len(checkpoints_list[0]) == len(checkpoints_list[1]), "The superresolution checkpoint num is not equal to the agent's."
        ...
        ignore_fields.extend(['sr_lr_scheduler', 'agent_lr_scheduler', 'constructor', 'net_type', 'actor_type', 'net_info'])
        ...
        for key in fields:
            ...
            elif key == 'sr_optimizer':
                self.sr_optimizer.load_state_dict(checkpoints_dict[0][key])
            elif key == 'agent_optimizer':
                self.agent_optimizer.load_state_dict(checkpoints_dict[1][key]) # the second model is the low level model
            else:
        ...
        if 'epoch' in fields:
            self.sr_lr_scheduler.last_epoch = self.epoch
            self.agent_lr_scheduler.last_epoch = self.epoch
